...
% idealized piston spherical source (uniform distribution of the vibrational 
...
xFieldBegin = -5e-3; %x, y, and z limits in m for the rectangular 3D simulation region
xFieldEnd   =  5e-3;
yFieldBegin = -5e-3;
yFieldEnd   =  5e-3;
zFieldBegin = 60e-3;
...
Transducer.radiusOfCurvature = radiusOfCurvature;
...
Rmax = F*(1+(1 - z/F).^2-2*(1 - z/F)*sqrt(1-(a/F)^2)).^0.5;
pFieldAnalytical = p0./(1 - z/F) .* (exp(-1i*k*z) - exp(-1i*k*Rmax));
pFieldAnalytical(abs(z-F)<eps) = 1i*p0*k*F*(1-sqrt(1-(a/F)^2))*exp(-1i*k*F);
